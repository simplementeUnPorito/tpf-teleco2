...
## Pruebas rápidas (sin Home Assistant)

Antes de probar desde Home Assistant, se puede verificar Node-RED publicando comandos directamente al broker MQTT local.

### Probar LED

```bash
mosquitto_pub -h localhost -t ha/lora/led/set -m "on"
mosquitto_pub -h localhost -t ha/lora/led/set -m "off"
```

### Probar cortina

```bash
mosquitto_pub -h localhost -t ha/lora/cortina/set -m "open"
mosquitto_pub -h localhost -t ha/lora/cortina/set -m "close"
```

Si Node-RED está bien configurado:

* se construye el JSON del downlink
* TTN lo encola
* el nodo lo recibe cuando abra su ventana RX (dependiendo del uplink del dispositivo)
